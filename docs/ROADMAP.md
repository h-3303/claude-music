...
**Phase 3b — direct requests and automatic filing** — *done 2026-09-17 (v0.5.0)*
- `request_music(items, playlist="Requests", download=True, min_confidence=0.85, ...)`: songs (`{"artist","title"}` / `"Artist - Title"`) and albums (`{"artist","album"}` / `"Artist - Album (album)"`) are appended to a persistent request playlist (source `request`, positions continue, JSPF rewritten), albums are expanded through MusicBrainz (`release?query=release:"X" AND artist:"Y"` then `release/{id}?inc=recordings+artist-credits+media`; plain official album with the fewest tracks wins over deluxe editions), single tracks resolved, the library scanned and diffed, then one `request` job runs `MatchJob` on just the new track ids, auto-approves `candidates` at or above `min_confidence` and queues them (folders in album mode). Job progress carries `phase`, `queued`, `total_mb`, `users`, `for_review`, `not_found`. No separate confirmation: the request is the instruction. `requester.py`, `MusicBrainzClient.find_release / release_tracks`, `Database.find_playlist / append_tracks / relocate`, `MatchJob.run(track_ids=...)`.
- Automatic tidy of new tracks only: `Tidy.apply_new(paths)` retags and files just those files (never deletes, no sibling propagation, no settle check; held with a reason when the full tidy would delete the file, tags are missing, or the target exists; leftover images/booklets follow the album once the download folder holds no audio; emptied folders pruned; download reports filed). `sync_downloads` runs it on every track it marks done when `auto_tidy` (userConfig, env `CLAUDE_MUSIC_AUTO_TIDY`, default on) is set, relocates match rows (`Database.relocate`) and reindexes (`library.reindex_moved`). `tidy_new(paths=None)` is the manual route; without paths an incremental `scan_library(collect_new=True)` decides what is new, files younger than the settle window are reported as `settling`. The monitor now says "tidy_new files it" for finished transfers that belong to no playlist.
