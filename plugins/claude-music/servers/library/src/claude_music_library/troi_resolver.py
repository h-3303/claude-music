...
fuzzy artist + title (needs nmslib). claude-music feeds it a JSPF of the tracks still missing and reads the
...
class Troi:
    ...
    def write_query(path: Path, rows) -> int:
        ...
        path.write_text(json.dumps({"playlist": {"title": "claude-music query", "track": tracks}}, ensure_ascii=False))
        return len(tracks)
