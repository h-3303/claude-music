...
def main(argv=None):
    """``claude-music-tidy analyse|apply [--force] [root]``: the same planner from a shell."""
    import sys
    ...
    root = args[1] if len(args) > 1 else os.environ.get("MUSIC_ROOT") or os.environ.get("CLAUDE_MUSIC_DIR") or "~/Music"
    ...
    try:
        ...
        if command == "analyse":
            ...
            sys.exit("usage: claude-music-tidy analyse | apply [--force] [root]")
