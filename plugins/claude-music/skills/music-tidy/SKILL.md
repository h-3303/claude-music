...
One library, one target state, one persistent decisions file. The `library` server's `tidy_analyse`
...
`music_dir` defaults to the plugin's configured library. Point it at a copy to rehearse. The same
planner is available from a shell as `claude-music-tidy analyse|apply [--force] [root]` inside the
library server's environment.
